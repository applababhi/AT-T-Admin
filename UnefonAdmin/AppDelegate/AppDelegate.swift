...
//  UnefonAdmin
//
//  Created by Shalini Sharma on 5/9/19.
...
class AppDelegate: UIResponder, UIApplicationDelegate, MessagingDelegate {
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        if let uuid:String = k_userDef.value(forKey: userDefaultKeys.user_Loginid.rawValue) as? String
        {
            if uuid != ""
            {
                // Call Api to push token
                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "pushTokenToServer"), object: nil, userInfo: nil)
            }
        }

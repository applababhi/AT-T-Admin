...
extension ForgotVC: UITableViewDataSource, UITableViewDelegate
{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if indexPath.row == 0
        {
            ...
            cell.tf.setPlaceHolderColorWith(strPH: "Ej: er793r")
...
extension ForgotVC
{
    func callSendCode()
    {
        ...
        let param: [String:Any] = ["supervisor_id":strUsername]
            ...
            else
            {
                if let internalCode:Int = json["internal_code"] as? Int
                {
                    ...
                    else
                    {
                        ...
                        if let msg:String = json["message"] as? String
                        {
                            self.showAlertWithTitle(title: "Unefon", message: msg, okButton: "Ok", cancelButton: "", okSelectorName: nil)
                            return
                        }
                    }
                }
    ...
    {
        self.showSpinnerWith(title: "Cargando...")
        let param: [String:Any] = ["supervisor_id":strUUID, "security_code": strSecurity, "new_password": strConfirm.md5Value]
      //  print(param)
        WebService.requestService(url: ServiceName.PUT_UpdatePassword.rawValue, method: .put, parameters: param, headers: [:], encoding: "URL", viewController: self) { (json:[String:Any], jsonString:String, error:Error?) in
            self.hideSpinner()
          //  print(jsonString)
            ...
            else
            {
                if let internalCode:Int = json["internal_code"] as? Int
                {
                    ...
                    else
                    {
                        ...
                        if let msg:String = json["message"] as? String
                        {
                            self.showAlertWithTitle(title: "Unefon", message: msg, okButton: "Ok", cancelButton: "", okSelectorName: #selector(self.back))
                        }
                    }
                }
            }
